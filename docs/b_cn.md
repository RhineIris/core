...
> **版本说明**：本技术文档是对”晶格认知模型（Lattice Cognitive Model, LCM）”项目书中核心模块的全面数学细化，涵盖从离散记忆格、生成头到训练损失的完整工程实现细节。v3.0 全面重写了码本更新机制（混合 EMA/梯度管理）、绑定格解绑方式（共轭乘法）、路由门控（Gumbel-Softmax）、对比格防崩（特征池）、低秩格参数化（纯梯度）以及线性注意力归一化，废除了 FSP 模块，并将推理过程剥离为独立零参数推理引擎（见 c.md），原推理解码器精简为轻量生成头。
...
生成头接收推理引擎的最终输出 `z_final`，将其线性投影为初始隐状态，然后进行自回归语言生成。它不从事实记忆中学习，仅负责语言的流畅性。

- **输入**：推理引擎输出 `z_final ∈ R^{B×d}`，线性投影为初始隐状态。
- **结构**：单层因果线性注意力 + GLU，共享词嵌入权重。
- **参数**：< 1M。
- **前向传播**：
  ```python
  def generation_head(z_final, max_len):
      hidden = linear_proj(z_final)          # 初始隐状态
      tokens = [bos_token]
...
推理过程由零参数推理引擎完成，详细规范见 `c.md`。生成头不参与推理循环。

核心接口：
- **输入**：`z_q ∈ R^{B×d}`（多格记忆体融合输出），可选接收 `z`（编码器原始输出）。
- **输出**：`z_final ∈ R^{B×d}`，作为生成头的初始隐状态。
...
- 生成头仅使用 `L_LM` 训练。推理引擎零参数，不参与损失计算。

### 6.2 参数更新规则（混合 EMA/梯度管理）
...
- 双曲流形格切空间 `T`
- 双码本对比格所有 `A`, `W` 矩阵
- 路由格码本 `C_route` 及投影 `W_route`
- 缩放因子 `α_i`
- 绑定格键值投影矩阵 `A_k`, `A_v`（复用低秩格共享基 `V`）
