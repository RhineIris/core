...
LCM 中，**系统一由多格记忆体实现**，海量显式记忆在格晶体中稳定储存，通过最近邻查找瞬间激活；**系统二由感知编码器、零参数推理引擎和轻量生成头实现**，负责动态上下文建模、基于纯数学操作的逐步推理与自然语言输出。
...
LCM 由**感知前端、多格记忆体、零参数推理引擎、生成头**四大模块首尾相连，形成认知闭环。
...
生成头是一个极轻量的自回归语言模型（单层线性注意力 + GLU），唯一作用是将推理引擎的最终输出向量 `z_final` 解码为自然语言 token。它不从事实记忆中学习，仅负责语言的流畅性和符合语法的序列生成。
